...
export default function ArticleComments(props) {
  ...
  if (articleComments.length === 0) {
    return <p>No comments have been posted.</p>
  }
        ...
        return (
          <h2 key={comments.comment_id}>
            <li className="comments-container">
              <p className="user-comments-username">User: {comments.author}</p>
                  
                     
            <div className="comment-body-votes-container">
              <p className="comment-text">{comments.body}</p>
              <p className="comment-data-votes">Votes: {comments.votes}</p>
              </div>
              
              <p className="comment-data-posted">Posted: {formatDate(comments.created_at)}</p>
